...
class DeleteAllCartItemsUseCaseUnitTest {

    private val repository: CartRepository = mockk()
    private lateinit var useCase: DeleteAllCartItemsUseCase

    @Before
    fun setUp() {
        useCase = DeleteAllCartItemsUseCase(repository)
    }

    @Test
    fun `invoke should return success from repository`() = runTest {
        coEvery { repository.deleteAllCartItems() } returns Resource.Success(Unit)

        val result = useCase()

        assertThat(result).isInstanceOf(Resource.Success::class.java)
        coVerify(exactly = 1) { repository.deleteAllCartItems() }
    }

    @Test
    fun `invoke should return error from repository`() = runTest {
        coEvery { repository.deleteAllCartItems() } returns Resource.Error("A local storage error occurred")
